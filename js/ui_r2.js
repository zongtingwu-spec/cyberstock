...
import { drawSparkline, drawCandlestick } from './chart_r2.js';
...
export function renderHoldings(rows, totalsObj, nameLookup) {
  ...
  if (!rows.length) {
    tbody.innerHTML = `<tr class="empty-row"><td colspan="10">尚無持股紀錄 — 點「+ 新增買入」開始</td></tr>`;
  } else {
    tbody.innerHTML = rows.map(r => {
      ...
      return `<tr data-sym="${r.symbol}">
        ...
        <td class="holding-spark-cell"><canvas class="holding-spark" data-sym="${r.symbol}" width="140" height="44"></canvas></td>
        <td><button class="del-btn" data-edit-sym="${r.symbol}">編輯</button></td>
      </tr>`;
    }).join('');

    // 畫每筆持股 sparkline (close prices,水平虛線標 avg cost)
    // 用 setTimeout 0 確保 table layout 後再繪 (rAF 在 headless 不穩)
    setTimeout(() => {
      for (const r of rows) {
        const canvas = tbody.querySelector(`.holding-spark[data-sym="${r.symbol}"]`);
        if (!canvas) continue;
        const candles = klineCache[r.symbol];
        if (!candles || candles.length < 2) {
          const ctx = canvas.getContext('2d');
          canvas.width = canvas.clientWidth || 140;
          canvas.height = canvas.clientHeight || 44;
          ctx.fillStyle = '#4a526e';
          ctx.font = '11px "Share Tech Mono", monospace';
          ctx.fillText('--', 4, canvas.height / 2 + 4);
          continue;
        }
        const closes = candles.map(c => c.c);
        drawSparkline(canvas, closes, { prev: r.avgPrice });
      }
    }, 0);
  }
...
const klineMeta = {};         // {sym: { fetchedAt, interval }}
const KLINE_STORAGE_KEY = 'cyberstock_klineCache_v2';

export function setKlineCache(sym, candles, interval = '1d') {
  klineCache[sym] = candles;
  klineMeta[sym] = { fetchedAt: Date.now(), interval };
}
...
export function getKlineMeta(sym) {
  return klineMeta[sym] || null;
}

/** 把 cache 寫進 localStorage,reload 後可秒開 */
export function persistKlineCache() {
  try {
    const payload = { cache: klineCache, meta: klineMeta };
    localStorage.setItem(KLINE_STORAGE_KEY, JSON.stringify(payload));
  } catch (e) {
    // 可能超過 quota, 不致命
    console.warn('[kline] persist failed:', e.message);
  }
}

/** 從 localStorage 載入 cache (首次 render 就有東西可畫) */
export function hydrateKlineCache() {
  try {
    const raw = localStorage.getItem(KLINE_STORAGE_KEY);
    if (!raw) return 0;
    const parsed = JSON.parse(raw);
    if (parsed.cache && parsed.meta) {
      Object.assign(klineCache, parsed.cache);
      Object.assign(klineMeta, parsed.meta);
      return Object.keys(klineCache).length;
    }
  } catch (e) {
    console.warn('[kline] hydrate failed:', e.message);
  }
  return 0;
}
