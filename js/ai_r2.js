...
/** 暫時性錯誤碼 (重試) vs 客戶端錯誤 (放棄) */
const TRANSIENT_STATUS = new Set([429, 500, 502, 503, 504, 529]);

/**
 * 重試包裝: 503/429/網路錯誤 自動 backoff retry
 * @param {() => Promise<Response>} doFetch  function returning fetch promise
 * @param {Object} opts  { maxRetries, onRetry }
 */
async function fetchWithRetry(doFetch, { maxRetries = 2, onRetry } = {}) {
  let lastErr = null;
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        ...
        onRetry?.(attempt + 1, 'network', delay);
        await new Promise(r => setTimeout(r, delay));
        continue;
      }
    }
...
export async function askClaude({ question, contextMarkdown, onChunk, onError, onDone, signal }) {
  ...
  const webSearch = !!settings.aiWebSearch;
  let res;
  try {
    res = await fetchWithRetry(() => fetch(ANTHROPIC_ENDPOINT, {
      method: 'POST',
      headers: {
        'x-api-key': key,
      ...
      signal,
    }), { onRetry: (n, st, d) => console.log(`[claude] retry ${n} (${st}) in ${d}ms`) });
  } catch (e) {
  ...
  if (!res.ok) {
    ...
    const friendly = res.status === 503 ? '⚠️ 伺服器忙線中 (重試 2 次仍失敗,過幾秒再問一次)\n\n'
                   : res.status === 429 ? '⚠️ 速率限制 (請稍候或檢查配額)\n\n'
                   : res.status === 401 ? '⚠️ API Key 錯誤或過期\n\n'
                   : res.status === 402 ? '⚠️ 帳戶餘額不足\n\n'
                   : '';
    onError?.(`${friendly}HTTP ${res.status}: ${body.slice(0, 800)}`);
    history.claude.pop();
    return;
  }
...
export async function askOpenAI({ question, contextMarkdown, onChunk, onError, onDone, signal }) {
  ...
  // 開啟搜尋時, 把 gpt-4o / gpt-4o-mini 自動換成 search-preview 版
  if (webSearch && model === 'gpt-4o') model = 'gpt-4o-search-preview';
  if (!key) { onError?.('未設定 OpenAI API Key'); return; }

  const sysText = SYSTEM_PROMPT_BASE + (contextMarkdown ? `\n\n${contextMarkdown}` : '');
  history.openai.push({ role: 'user', content: question });

  // o-series 與 search-preview 都不支援 streaming
  const isOSeries = /^o\d/i.test(model);
  const isSearchPreview = model.includes('search-preview');
  const noStream = isOSeries || isSearchPreview;

  let res;
  try {
    res = await fetchWithRetry(() => fetch(OPENAI_ENDPOINT, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        stream: !noStream,
        ...(noStream ? {} : { temperature: 0.6, max_tokens: 2048 }),
      }),
      signal,
    }), { onRetry: (n, st, d) => console.log(`[openai] retry ${n} (${st}) in ${d}ms`) });
  } catch (e) {
    onError?.(`網路錯誤: ${e.message}`);
    history.openai.pop();
    return;
...
export async function askGemini({ question, contextMarkdown, onChunk, onError, onDone, signal }) {
  ...
  let res;
  try {
    res = await fetchWithRetry(() => fetch(url, {
      method: 'POST',
      ...
      signal,
    }), { onRetry: (n, st, d) => console.log(`[gemini] retry ${n} (${st}) in ${d}ms`) });
  } catch (e) {
    onError?.(`網路錯誤: ${e.message}`);
    history.gemini.pop();
    return;
...
async function synthesizeViaClaude({ userMsg, sysFull, settings, onChunk, onError, onDone, signal }) {
  const res = await fetchWithRetry(() => fetch(ANTHROPIC_ENDPOINT, {
    method: 'POST',
    headers: {
    ...
    signal,
  }), { onRetry: (n, st, d) => console.log(`[synth-claude] retry ${n} (${st}) in ${d}ms`) })
...
async function synthesizeViaOpenAI({ userMsg, sysFull, settings, onChunk, onError, onDone, signal }) {
  ...
  const isOSeries = /^o\d/i.test(model);
  const res = await fetchWithRetry(() => fetch(OPENAI_ENDPOINT, {
    method: 'POST',
    headers: {
    ...
    signal,
  }), { onRetry: (n, st, d) => console.log(`[synth-openai] retry ${n} (${st}) in ${d}ms`) })
    .catch(e => ({ ok: false, _err: e.message }));
...
async function synthesizeViaGemini({ userMsg, sysFull, settings, onChunk, onError, onDone, signal }) {
  ...
  const res = await fetchWithRetry(() => fetch(url, {
    ...
    signal,
  }), { onRetry: (n, st, d) => console.log(`[synth-gemini] retry ${n} (${st}) in ${d}ms`) })
    .catch(e => ({ ok: false, _err: e.message }));
